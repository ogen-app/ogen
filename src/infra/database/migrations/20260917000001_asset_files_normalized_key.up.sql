...
-- latest non-null normalized key from image_extractions (one row per asset, newest
...
-- the derivative instead of the undecodable original.
UPDATE asset_files af
SET normalized_s3_key = latest.normalized_s3_key,
    thumbnail_s3_key  = COALESCE(af.thumbnail_s3_key, latest.normalized_s3_key)
FROM (
