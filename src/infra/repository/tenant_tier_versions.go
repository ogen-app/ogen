...
type RetireOptions struct {
	...
	// grandfathering onto the now-retired version.
	Force bool
	// ReassignToVersionID, when non-empty, migrates every live assignment onto
	// that (different, active) version before retiring, in the same transaction.
	ReassignToVersionID string
}
...
type TenantTierVersionRepository interface {
	...
	// reassignment was requested). When open assignments remain: a set
	// ReassignToVersionID migrates them onto that (different, active) version in
...
func (r *tenantTierVersionRepository) DeleteDraft(ctx context.Context, id string) error {
	return r.db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		status, err := versionStatusForUpdateTx(ctx, tx, id)
		if err != nil {
			return err
		...
		// Price rows cascade via the FK; the immutability trigger permits deleting
		// a draft row and is the backstop if this ever ran against a published one.
		_, err = tx.NewDelete().Model((*models.TenantTierVersion)(nil)).
			Where("id = ?", id).Exec(ctx)
		return err
	})
}

func (r *tenantTierVersionRepository) Retire(ctx context.Context, id string, opts RetireOptions, at time.Time) (int, error) {
	reassigned := 0
	err := r.db.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		reassigned = 0 // reset on retry
		status, err := versionStatusForUpdateTx(ctx, tx, id)
		if err != nil {
			return err
		}
		...
		switch {
		case opts.ReassignToVersionID != "":
			n, err := migrateOpenAssignmentsTx(ctx, tx, id, opts.ReassignToVersionID, at)
			if err != nil {
				return err
			}
			reassigned = n
...
func migrateOpenAssignmentsTx(ctx context.Context, tx bun.Tx, fromID, targetID string, at time.Time) (int, error) {
	if targetID == fromID {
		return 0, ErrReassignTargetIsSelf
	}
	var targetTier, targetStatus string
	err := tx.NewSelect().Model((*models.TenantTierVersion)(nil)).
		Column("tier_id", "status").Where("id = ?", targetID).For("UPDATE").Scan(ctx, &targetTier, &targetStatus)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return 0, ErrReassignTargetNotFound
		}
		return 0, err
	}
	if targetStatus != models.TierVersionStatusActive {
		return 0, ErrReassignTargetNotActive
	}

	var tenantIDs []string
	if err := tx.NewSelect().Model((*models.TenantTierAssignment)(nil)).
		Column("tenant_id").
		Where("tier_version_id = ?", fromID).Where("upper_inf(valid)").
		Order("tenant_id").
	...
	for _, tid := range tenantIDs {
		// Point the tenant at the target's tier (the assignment and tier_id stay in
		// step). No deleted_at filter: a migrated assignment must match tier_id even
		// for a soft-deleted tenant, so the retired version ends with zero live rows.
		if _, err := tx.NewUpdate().Model((*models.Tenant)(nil)).
			Set("tier_id = ?", targetTier).
			Set("updated_at = ?", at).
			Where("id = ?", tid).Exec(ctx); err != nil {
			return 0, err
		}
		// Close the open range (the append-only trigger's one permitted update).
		if _, err := tx.NewUpdate().Model((*models.TenantTierAssignment)(nil)).
			Set("valid = tstzrange(lower(valid), ?, '[)')", at).
			Where("tenant_id = ?", tid).Where("tier_version_id = ?", fromID).Where("upper_inf(valid)").
			Exec(ctx); err != nil {
			return 0, err
		}
		...
		a := &models.TenantTierAssignment{ID: aid, TenantID: tid, TierVersionID: targetID, Reason: models.AssignmentReasonOperatorSet, CreatedAt: at}
...
func (r *tenantTierVersionRepository) TenantsOnVersion(ctx context.Context, versionID string, limit, offset int) ([]models.VersionAssignment, error) {
	var rows []models.VersionAssignment
	err := r.db.NewSelect().Model((*models.TenantTierAssignment)(nil)).
		ColumnExpr("tta.tenant_id AS tenant_id").
		ColumnExpr("COALESCE(tn.name, '') AS tenant_name").
		ColumnExpr("lower(tta.valid) AS valid_from").
		Join("LEFT JOIN tenants AS tn ON tn.id = tta.tenant_id").
		Where("tta.tier_version_id = ?", versionID).
		Where("upper_inf(tta.valid)").
		OrderExpr("lower(tta.valid) ASC, tta.tenant_id ASC").
		Limit(limit).
		Offset(offset).
		Scan(ctx, &rows)
	if err != nil {
