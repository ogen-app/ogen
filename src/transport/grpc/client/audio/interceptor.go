...
// Correlation metadata keys. They MUST match audio-service's
...
// boundary: API request -> River job -> audio-service (CON-111). This is
...
// outgoing gRPC metadata so audio-service enriches its logs with the same ids
...
// call (all three audio RPCs are unary, as are health checks).
